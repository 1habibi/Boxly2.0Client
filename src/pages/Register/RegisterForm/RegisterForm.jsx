...
export const RegisterForm = ({ onFinish, isLoading }) => {
  return (
    <Form style={{ width: "100%" }} name="register" onFinish={onFinish}>
      <Form.Item
        name="email"
        hasFeedback
        rules={[
      ...
      <Form.Item
        ...
        rules={[
          ...
          {
            pattern: /^.*(?=.{8,})(?=.*[a-zA-Z])(?=.*\d)(?=.*[!#$%&? "]).*$/,
            message: "Длинна пароля - минимум 8 символов. Должен содержать большие и маленькие буквы, цифры, а также символы",
          },
        ]}
      >
